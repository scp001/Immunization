...
//Rare exceptions to indicate a bug in the _immunization code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_immunization = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_immunization]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.immunization.module"
